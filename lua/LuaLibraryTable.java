...
enum LuaLibraryTable implements BiConsumer<Environment, LuaObject>, LuaMethod
{
	concat() {
		@Override
		public LuaObject call(LuaObject[] args)
		{
			...
			while(i.doLE(j).getBoolean())
			{
				sb.append(args[0].rawGet(i));
				
				if(i.doLT(j).getBoolean())
	...
	insert() {
		...
		public LuaObject call(LuaObject[] args)
		{
			...
			LuaObject pos = args.length > 2 ? args[1] : len.doAdd(LuaInteger.ONE);
			LuaObject value = args.length > 2 ? args[2] : args[1];

			while(pos.doLE(len).getBoolean())
			{
				args[0].rawSet(len.doAdd(LuaInteger.ONE), args[0].rawGet(len));
				len = len.doSub(LuaInteger.ONE);
			}
	...
	remove() {
		...
		public LuaObject call(LuaObject[] args)
		{
			...
			while(pos.doLE(len).getBoolean())
			{
				pos = pos.doAdd(LuaInteger.ONE);
				args[0].rawSet(pos.doSub(LuaInteger.ONE), args[0].rawGet(pos));
			}
			return curr;
		}
	},
		...
		public LuaObject call(LuaObject[] args)
		{
			...
			for(long i = 1; i < len; i++)
			{
				for(long j = i; j <= len; j++)
				{
					if(comp == null)
					{
						swap = !tbl.rawGet(LuaInteger.valueOf(i)).doLT(tbl.rawGet(LuaInteger.valueOf(j))).getBoolean();
					}
					else
					{
						Object o = comp.doCall(new LuaObject[] { tbl.rawGet(LuaInteger.valueOf(i)), tbl.rawGet(LuaInteger.valueOf(j)) });
					
						if(o instanceof LuaObject[])
							o = ((LuaObject[]) o)[0];
						else
							o = ((Lua.LuaValue) o).evaluate();
