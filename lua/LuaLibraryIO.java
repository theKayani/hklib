...
enum LuaLibraryIO implements BiConsumer<Environment, LuaObject>, BiFunction<LuaInterpreter, LuaObject[], LuaObject>
{
	close() {
		@Override
		...
		public LuaObject apply(LuaInterpreter interp, LuaObject[] args)
		{
			if(args.length > 0)
			{
				...
				if(args[0] instanceof LuaFileUserdata)
					...
					fud = open.apply(interp, new LuaObject[] { args[0], new LuaString("r") });
	...
	lines() {
		@Override
		public LuaObject apply(LuaInterpreter interp, final LuaObject[] args)
		{
			if(args.length > 0)
			{
				final LuaFileUserdata fud = (LuaFileUserdata) open.apply(interp, new LuaObject[] { args[0], new LuaString("r") });
			
				return new LuaFunction()
				{
					@Override
					LuaObject doCall(LuaObject[] args2)
					{
						LuaObject obj = fud.lines(args, 1).doCall(new LuaObject[0]);
						if(obj.isNil())
						{
							try
							{
		...
		public LuaObject apply(LuaInterpreter interp, LuaObject[] args)
		{
			if(args.length > 0)
			{
				...
				if(args[0] instanceof LuaFileUserdata)
					...
					fud = open.apply(interp, new LuaObject[] { args[0], new LuaString("w") });
...
//	},
	...
	static
	{
		ioMetatable.setIndex("__name", new LuaString("FILE*"));
		ioMetatable.setIndex("__index", ioMetatable);

		ioMetatable.setIndex("close", new LuaFunction() {
			@Override
		...
		ioMetatable.setIndex("flush", new LuaFunction() {
			@Override
			LuaObject doCall(LuaObject[] args)
			{
				Lua.checkArgs(toString(), args, LuaType.USERDATA);
		...
		ioMetatable.setIndex("read", new LuaFunction() {
			@Override
			LuaObject doCall(LuaObject[] args)
			{
				Lua.checkArgs(toString(), args, LuaType.USERDATA);
		...
		ioMetatable.setIndex("write", new LuaFunction() {
			@Override
			LuaObject doCall(LuaObject[] args)
			{
				Lua.checkArgs(toString(), args, LuaType.USERDATA);
		...
		ioMetatable.setIndex("lines", new LuaFunction() {
			@Override
