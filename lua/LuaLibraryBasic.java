...
enum LuaLibraryBasic implements BiConsumer<Environment, LuaObject>, LuaMethod
{
	_assert() {
		@Override
...
//			throw new Error();
	...
	dofile() {
		private LuaObject apply(LuaInterpreter interp, LuaObject[] args)
		{
			...
			try
			{
				...
				Object res = interp.readLua(new FileReader(file), file.getPath(), interp.global, true).execute();
	...
	load() {
		private LuaObject apply(LuaInterpreter interp, LuaObject[] args)
		{
			...
			try
			{
				final LuaChunk chunk = interp.readLua(new StringReader(args[0].getString()), Lua.STDIN, interp.global, true);
				...
				return new LuaFunction()
				{
					...
					LuaObject doCall(LuaObject[] args)
					{
						Object res = chunk.execute((Object[]) args);
	...
	loadfile() {
		private LuaObject apply(LuaInterpreter interp, LuaObject[] args)
		{
			...
			{
				File file = new File(args[0].getString());
				final LuaChunk chunk = interp.readLua(new FileReader(file), file.getPath(), interp.global, true);
			...
			if(!tbl.map.isEmpty())
			{
				...
				else
				{
					...
					while(itr.hasNext())
					{
						...
						if(!ent.getKey().doEQ(arg2).getBoolean())
							continue;
	...
	pcall() {
		private LuaObject call(LuaInterpreter interp, LuaObject[] args)
		{
			...
			try
			{
				LuaObject res = args[0].doCall(tmp);
		...
		{
			final LuaInterpreter interp = env.interp;
			table.rawSet(new LuaString(toString()), new LuaFunction() {
				@Override
				LuaObject doCall(LuaObject[] args)
				{
					return call(interp, args);
				}
			});
		}
	},
