...
for _, funcName in ipairs(
    ...
    local typeExpected = funcName:match("^assertIs([A-Z]\\w*)$")
...
for _, funcName in ipairs(
    ...
    local typeUnexpected = funcName:match("^assertNotIs([A-Z]\\w*)$")
    -- Lua type() always returns lowercase, also make sure the match() succeeded
...
function TapOutput:updateStatus( node )
    if node:isSkipped() then
        print("ok ", self.result.currentTestNumber, "\t# SKIP ", node.msg, "\n" )
        return
    end
